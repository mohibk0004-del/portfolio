...
export const AnimatedThemeToggle = ({ className, isDark, onToggle }: { className?: string, isDark?: boolean, onToggle?: () => void }) => {
    const [localTheme, setLocalTheme] = useState("light");
    const isCurrentlyDark = isDark !== undefined ? isDark : localTheme === "dark";

    const handleToggle = () => {
        if (onToggle) {
            onToggle();
        } else {
            setLocalTheme(isCurrentlyDark ? "light" : "dark");
        }
    };
    ...
    return (
        <Button
            ...
            className={cn(
                "px-2.5 transition-all duration-200 hover:scale-110 active:scale-90 bg-transparent border-transparent text-[color:var(--text)] hover:shadow-[0_0_14px_var(--matrix-glow)]",
                className
            )}
            variant="ghost"
            size="icon"
        >
            <SolarSwitch isDark={isCurrentlyDark} />
        </Button>
    );
};
